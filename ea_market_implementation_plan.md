...
```sql
-- Tính max drawdown trong 30 ngày gần nhất (thay X = login của eaCode)
WITH cumulative AS (
  SELECT
    closeTime,
...
running_max AS (
  SELECT
    cum_pnl,
    MAX(cum_pnl) OVER (ORDER BY closeTime ROWS UNBOUNDED PRECEDING) AS peak
  FROM cumulative
)
SELECT MIN(cum_pnl - peak) AS max_drawdown_abs
FROM running_max;
```

**drawdown_score** = `1 - (|max_drawdown_abs| / initial_balance)`  
→ Nếu DD > 70% initial_balance → kích hoạt **Hard Kill** ngay lập tức.

#### Metric 2: Loss Acceleration

```sql
-- X = login của eaCode đang xét
...
WHERE rn <= (
  ...
  FROM (
    ...
    FROM Orders WHERE login = X AND closeTime IS NOT NULL
  ) t
  WHERE profit > 0
);
```
...
```sql
-- Tính survival_score của eaCode 'EA_GOLD_001' (login=X) tại regime 'trend_low'
SELECT
  r.eaCode,
